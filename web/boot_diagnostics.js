...
(function () {
	...
	// Reads a few pixels straight out of Godot's WebGL context, right after the engine's
	// own rAF callback returns and therefore before the frame is composited. This is the
	// one measurement that separates "Godot is not drawing" from "Safari is not
	// presenting what Godot drew" — every other probe sees the same thing either way.
	// getContext() with no attributes returns the context Godot already created.
	var glCtx = null;
	var pixHash = -1;
	var pixReads = 0;
	var pixChanges = 0;
	function samplePixels() {
		try {
			...
			if (!glCtx) {
				glCtx = cv.getContext('webgl2') || cv.getContext('webgl');
				if (!glCtx) {
					return;
				}
			}
			...
			pixHash = s;
		} catch (e) { /* context lost, or read not permitted: stay silent */ }
	}
	...
	function diagnostics() {
		...
		var lines = [
			...
			'PIXELS   reads ' + pixReads + '  changed ' + pixChanges
				+ (pixReads > 20 && pixChanges === 0 ? '   <-- ENGINE NOT DRAWING' : ''),
	...
	function boot() {
		...
		// A/B control for the compositing fix: ?d=nofix strips the layer promotion so
		// the portrait freeze should come back. If it does not, the CSS was not what
		// fixed it and we are looking at the wrong thing again.
		if (q.indexOf('d=nofix') !== -1 && c) {
			c.style.transform = 'none';
			c.style.webkitTransform = 'none';
		}

		// Belt and braces for the same bug: if presentation ever stalls again, a real
		// size change is the one thing Godot's updateSize() reacts to. Rotating the
		// phone is what a user would stumble into; this makes it automatic.
		if (window.visualViewport) {
			window.visualViewport.addEventListener('resize', function () {
				if (c) {
					c.style.transform = c.style.transform === 'none'
						? 'none' : 'translateZ(0)';
				}
			});
		}
		// iOS plays correctly with ?debug=1 and is frozen without it, so something in
		// the overlay path is the accidental cure. These modes isolate the candidates,
		// one variable each, so a tester only has to answer "can you tap New Game?".
		//
		//   ?d=late     no overlay, no timer — panel appears after 20 s showing what
		//               the engine was doing WHILE frozen (the reading we cannot
		//               otherwise capture, because observing it cures it)
		//   ?d=overlay  overlay element only, no periodic timer
		//   ?d=rect     periodic canvas.getBoundingClientRect(), no overlay
		...
		// (?d=noop fails), so it is something render() *does*. These two split it:
		//   ?d=text  the periodic DOM write only, no diagnostic reads
		//   ?d=read  the diagnostic reads only, no DOM write
		if (mode === 'text' || mode === 'read' || mode === 'both' || mode === 'estw') {
			passThrough = true;
			build('Split test', 'Try to play. Ignore these numbers.');
		}
		...
		if (mode === 'full') {
			...
			setInterval(function () {
				if (navigator.storage && navigator.storage.estimate) {
					navigator.storage.estimate().then(function () {}, function () {});
				}
			}, 1000);
		} else if (mode === 'noop') {
			setInterval(function () { /* deliberately empty */ }, 1000);
		} else if (mode === 'text') {
			var n = 0;
			setInterval(function () {
				if (pre) {
					n += 1;
					pre.textContent = 'tick ' + n;   // DOM write, nothing read
				}
			}, 1000);
		} else if (mode === 'read') {
			setInterval(function () {
				diagnostics();                       // reads only, result discarded
			}, 1000);
		} else if (mode === 'keep') {
			// Everything ?debug=1 does, minus visibility. If this works it is directly
			// shippable as a permanent keepalive, understanding can come afterwards.
			// If it fails while ?debug=1 works, the *visible* panel is what matters,
			// which points at compositing rather than at anything render() computes.
			var host = document.createElement('div');
			host.setAttribute('style', 'position:fixed;left:-9999px;top:0;width:1px;'
			...
			// render() minus the async storage.estimate(): reads + one sync DOM write.
			setInterval(function () {
				if (pre) {
			...
			// Only render()'s async tail: estimate(), then write from the callback.
			setInterval(function () {
				if (pre && navigator.storage && navigator.storage.estimate) {
					navigator.storage.estimate().then(function (e) {
						pre.textContent = 'used ' + (e && e.usage);
					}, function () {});
				}
			}, 1000);
		} else if (mode === 'nudge') {
			...
			setTimeout(function () {
				passThrough = true;
				build('Frozen-state reading',
					'These numbers cover the last 20 s, while it was stuck.');
			}, 20000);
		}
		watch();
	}
