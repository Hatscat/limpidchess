...
/** @type {string} */
const CACHE_VERSION = '1784304006|3634891';
/** @type {string} */
...
/** @type {string[]} */
const CACHEABLE_FILES = ["stockfish-18-lite-single.js","stockfish-18-lite-single.wasm","index.png","index.wasm","index.pck"];
	...
	(event) => {
		...
		if (isNavigate || isCacheable) {
			event.respondWith((async () => {
				...
				const scopePath = new URL(self.registration.scope).pathname; const navPath = new URL(event.request.url).pathname; let cached = await cache.match(navPath === scopePath ? CACHED_FILES[0] : event.request, { ignoreSearch: true });
				if (cached != null) {
					if (ENSURE_CROSSORIGIN_ISOLATION_HEADERS) {
